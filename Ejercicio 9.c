...
struct Pares{
	int x;
	int y;
	operacion operador_entre_elementos;
};
...
int ejecutar_operacion(struct Pares *tupla){
	return tupla->operador_entre_elementos(tupla->x, tupla->y);
}
...
int main(){
	...
	listado_tuplas[0].operador_entre_elementos = menor;
	
	listado_tuplas[1].x = 1;
	listado_tuplas[1].y = -120;
	listado_tuplas[1].operador_entre_elementos = mayor;
	
	listado_tuplas[2].x = 45;
	listado_tuplas[2].y = 56;
	listado_tuplas[2].operador_entre_elementos = suma;
	
	for(i = 0; i < 3; i++){
		resultado = ejecutar_operacion(&listado_tuplas[i]);
